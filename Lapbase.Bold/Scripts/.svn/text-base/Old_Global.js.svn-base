...
function ShowDivMessage(strMessage, AutoHidden)
{
	...
	if (AutoHidden)
	    window.setTimeout("javascript:HideDivMessage();", 2000, "javascript");
    return;
}
...
function FetchFieldsCaption_CallBack()
{
    ...
    if(XmlHttp.readyState == 4)  {
        //if (document.getElementById("txtTest")) document.getElementById("txtTest").value = XmlHttp.responseText;
...
function LoadFloodingDiv_ImageVideo(strFileName, FileType){
    ...
    switch(FileType)
    {
        case 1 : // Image Document
            ...
            imgDoc.style.height = (divHeight-30) + "px";
...
function SetPatientTitleEvents(){
    if (document.getElementById("tblPatientTitle_cmbGroup_CodeList"))
        document.getElementById("tblPatientTitle_cmbGroup_CodeList").onchange = function(){
            UpdateGroupTargetWeight();
        }
        
    if (document.getElementById("tblPatientTitle_txtTargetWeight_txtGlobal"))
        document.getElementById("tblPatientTitle_txtTargetWeight_txtGlobal").onchange = function(){
            UpdateGroupTargetWeight();
        }
    return;
}
...
function InitializePage(strAppAchemaID){
    ...
    iTimerID = window.setInterval("IsTitlesLoaded( );", 100,"javascript");
...
function InitializePage_FloatingDiv(strAppAchemaID){
    iTimerID = window.setInterval("IsTitlesLoaded_FloatingDive( );", 100,"javascript");
    FetchFieldsCaption(false,  document.getElementById("txtHCulture").value, strAppAchemaID);
}
...
function IsFloadingDivLoaded(strAppAchemaID){
    iTimerID = window.setInterval("FloadingDivLoaded('" + strAppAchemaID + "')", 100,"javascript");
}

...
<xsl:template match="/">
    ...
    <body>
      ...
      <table style="width:97%" cellpadding = "0" cellspacing = "0" border="0" id ="tblDoctorXSLT" class="testNameTable">
        <xsl:for-each select ="/dsSchema/tblGroup">
        <tr >
          ...
          <xsl:attribute name ="onclick">javascript:tblGroupXSLT_onclick('<xsl:value-of select="Code"/>', '<xsl:value-of select="Description"/>');</xsl:attribute>

...
public partial class _Default : Lapbase.Business.BasePage
{
    ...
    #region private void GetChangeLog()
    private void GetChangeLog()
    {
        lblUserAgreement.Text = "<b>Note to users: </b><br/>By accessing Lapbase you undertake to comply with and are bound by the Lapbase Licencing and Services Agreement. <br/>See <a href='http://www.lapbase.com/about.php'>www.lapbase.com</a> for a copy.";

        System.Data.SqlClient.SqlCommand cmdSelect = new System.Data.SqlClient.SqlCommand();
        System.Data.DataSet dsVersion = new System.Data.DataSet();

        gClass.MakeStoreProcedureName(ref cmdSelect , "sp_VersionLog", true);
        dsVersion = gClass.FetchData(cmdSelect, "tblVersionLog");
        String versionNo = "";
        String changeLog = "";

        if ((dsVersion.Tables.Count > 0) && (dsVersion.Tables[0].Rows.Count > 0))
        {
            versionNo = dsVersion.Tables[0].Rows[0]["VersionNo"].ToString();
            if (versionNo != "")
                versionNo = "<b>Version: " + versionNo + "</b>";
            lblChangeLog.Text = versionNo;
            
            changeLog = dsVersion.Tables[0].Rows[0]["ChangeLog"].ToString();
            if (changeLog != "")
                changeLog = "<br><br><b>Change Log:</b><br>" + changeLog ;

            lblChangeLog.Text = versionNo + changeLog;
    ...
    private void SettingCookies()
    {
        ...
        Response.SetCookie(new HttpCookie("Feature", ""));
    ...
    #region private void CheckInactive()
    private void CheckInactive()
    {
        System.Data.SqlClient.SqlCommand cmdSelect = new System.Data.SqlClient.SqlCommand();
        System.Data.DataSet dsOrganization = new System.Data.DataSet();
        cmdSelect.Parameters.Add("@OrganizationName", SqlDbType.VarChar, 100).Value = Request.Url.Host.ToLower();
        gClass.MakeStoreProcedureName(ref cmdSelect , "sp_CheckOrganization", true);
        dsOrganization = gClass.FetchData(cmdSelect, "tblOrganization");

        if ((dsOrganization.Tables.Count > 0) && (dsOrganization.Tables[0].Rows.Count > 0))
        {
            if (Convert.ToBoolean(dsOrganization.Tables[0].Rows[0]["InactiveFlag"].ToString()) == true)
            {
                Response.Redirect("InactivePractice.aspx");
            }
        }
    }
    #endregion

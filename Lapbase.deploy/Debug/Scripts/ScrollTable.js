...
function ResizeCells(table, cornerDiv, headerRowDiv, headerColumnDiv, bodyDiv)
{
	...
	// START - Steele's new code

	var cornerColumns = new Array();
	var arrayIndex = 0;
	var numCornerCols = cornerDiv.childNodes[childElement].childNodes[childElement].childNodes[0].childNodes.length;

	for ( i=0; i < numCornerCols; i++)
	{
		//alert( "Testing Node" );
		var tag = cornerDiv.childNodes[childElement].childNodes[childElement].childNodes[0].childNodes[i].tagName;
		if (tag == "TH" || tag == "TD")
		{
			//alert( "found header element or content element" )
			cornerColumns[arrayIndex] = cornerDiv.childNodes[childElement].childNodes[childElement].childNodes[0].childNodes[i];
			arrayIndex++;
		}

	}

	//alert( "Corner: saving " + cornerColumns.length );

	var headerColumnColumns = new Array();
	headerColumnColumns = headerColumnDiv.childNodes[childElement].childNodes[childElement].childNodes[0].childNodes;

	//alert( "HeaderColumn: saving " + headerColumnColumns.length );

	for ( i=0; i<cornerColumns.length; i++)
	{
		SetWidth( cornerColumns[i], headerColumnColumns[i] );
	}

	//alert( "After setWidth()" );

	// END - Steele's new code
